...
namespace GR
{
	namespace Dx12
	{
		...
		bool GraphicsDevice::CreateMeshDataFromGenerator(const GeometryGenerator::MeshData& data, Mesh* mesh)
		{
			if (m_LoadCommander[MESH_LOADER] == nullptr)
				m_LoadCommander[MESH_LOADER] = GetGraphicsCommander();

			mesh->CreateMesh(m_LoadCommander[MESH_LOADER], data.Vertices, data.Indices);
			return true;
		}
