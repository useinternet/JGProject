	...
	namespace Dx12
	{
		...
		RootSignature RootSignatureCache::GetRootSignature(ERootSignature enumpass)
		{
			...
			case ERootSignature::G_Common:
				...
				rootSig.InitShaderResourceView(0, 4);
				...
				break;
			case ERootSignature::G_ShadowMapping:
				rootSig.InitDescriptorTable(RootSignature::SRV, 1, 0, 0, D3D12_SHADER_VISIBILITY_PIXEL);
				rootSig.InitDescriptorTable(RootSignature::SRV, 1, 1, 0, D3D12_SHADER_VISIBILITY_PIXEL);
				rootSig.InitDescriptorTable(RootSignature::SRV, 1, 0, 1, D3D12_SHADER_VISIBILITY_PIXEL);
				rootSig.InitShaderResourceView(0, 2, D3D12_SHADER_VISIBILITY_PIXEL);
				rootSig.InitShaderResourceView(0, 3, D3D12_SHADER_VISIBILITY_PIXEL);
				rootSig.InitConstantBufferView(0, 0, D3D12_SHADER_VISIBILITY_PIXEL);
				rootSig.AddStaticSampler(0, RootSignature::LINEAR_WRAP);
				rootSig.AddStaticSampler(1, RootSignature::SHADOW);
				break;
