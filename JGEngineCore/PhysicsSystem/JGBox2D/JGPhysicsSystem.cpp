...
#include"JGDynamics/JGContactListener.h"
...
JGPhysicsSystem::JGPhysicsSystem()
{
	...
	m_Listener  = make_unique<JGContactListener>();
}
...
JGPhysicsWorld* JGPhysicsSystem::AddPhysicsWorld(const wstring& WorldName,JGVector2D& Gravity)
{
	...
	unique_ptr<JGPhysicsWorld> pyWorld = make_unique<JGPhysicsWorld>(m_Listener.get(),m_DebugDraw.get(),Gravity);
	result = pyWorld.get();
