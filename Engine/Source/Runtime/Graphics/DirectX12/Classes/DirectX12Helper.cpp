...
HDX12ComPtr<HDX12CommandAllocator> CreateD3DCommandAllocator(HDX12ComPtr<HDX12Device> device, D3D12_COMMAND_LIST_TYPE type)
{
	HRESULT hr = S_OK;
...
HDXC12Blob* HDirectX12Helper::CompileShader(const PString& filePath, const PString& sourceCode, const PString& entry, const PString& target, PString* out_errorMsg)
{
	...
	HRESULT hResult = pCompiler->Compile(pTextBlob, filePath.GetRawWString().c_str(), entry.GetRawWString().c_str(), target.GetRawWString().c_str(), nullptr, 0, nullptr, 0,
