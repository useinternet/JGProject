...
namespace JG
{
	...
	{
	private:
        UniquePtr<Zep::ZepEditor_ImGui> mZepEditor;
        std::function<void(std::shared_ptr<Zep::ZepMessage>)> mCallback;
        JVector2 mEditorSize;
        bool     mIsChildWindow = false;
    public:
        JGScriptEditor(const JVector2& editorSize, bool isChildWindow = true,  const JVector2& pixelScale = JVector2(1,1));

        void Load(const String& fileName, const String& text);
