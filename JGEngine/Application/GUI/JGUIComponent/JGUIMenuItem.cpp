...
void JGUIMenuItem::Awake()
{
	...
	m_NameText = CreateJGUIComponent<JGUIText>("MenuItemNameTxt");
	m_ShortCutText = CreateJGUIComponent<JGUIText>("MenuItemShortCutTxt");
	m_BackGround = CreateJGUIComponent<JGUIRectangle>("SubItemBacground");
	...
	m_BackGround->RegisterCollider(GetOwnerWindow(), JGUI_Collider_Box);
...
void JGUIMenuItem::AddSeparator()
{
	...
	auto separtor = CreateJGUIComponent<JGUIRectangle>("Separtor_" + to_string(idx));
...
bool JGUIMenuItem::CollisionCheck(const JVector2Int& mouse_pos)
{
	...
	return m_BackGround->GetCollider()->CheckInPoint(mouse_pos);
}
