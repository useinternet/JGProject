...
void JGUIDock::Awake()
{
	m_DockTarget = CreateJGUIComponent<JGUIComponent>("DockTarget", JGUI_ComponentFlag_TopMost);
...
void JGUIDock::SettingDockInfo()
{
	...
	resize_rect.Demical();
	std::vector<JGUIDockResizeLine*> vEntryResizeLines;
	std::vector<JGUIDockResizeLine*> hEntryResizeLines;
	...
	std::map<JGUIDockResizeLine*, bool> check_map;
	std::map<JGUIWindow*, bool>         check_win;


	std::sort(vEntryResizeLines.begin(), vEntryResizeLines.end(), [&](JGUIDockResizeLine* l1, JGUIDockResizeLine* l2)
	{
		return l1->GetTransform()->GetLocalPosition().x < l2->GetTransform()->GetLocalPosition().x;
	});
	std::sort(hEntryResizeLines.begin(), hEntryResizeLines.end(), [&](JGUIDockResizeLine* l1, JGUIDockResizeLine* l2)
	{
...
void JGUIDock::SettingDockInfo(int type, 
...
{ 
	...
	if (type == 0)
	{
		...
		float pos_ratio = line_world_pos.y / prev_rect.height();
	
		auto top_area = owner->TopArea();
		auto bottom_area = owner->BottomArea();

		  
		prevTopRect = prev_rect;
		prevTopRect.top    = top_area.top;
		prevTopRect.bottom = top_area.bottom;
		prevBottomRect = prev_rect;
		prevBottomRect.top = bottom_area.top;
		prevBottomRect.bottom = bottom_area.bottom;
		...
		vector<JGUIDockResizeLine*> l_t_line_array;
		l_t_line_array.assign(
			owner->m_ConnectedResizeLines[DD_Top].begin(),
			owner->m_ConnectedResizeLines[DD_Top].end());
		sort(l_t_line_array.begin(), l_t_line_array.end(), [&](JGUIDockResizeLine* l1, JGUIDockResizeLine* l2) -> bool
		{
			return l1->GetTransform()->GetLocalPosition().x < l2->GetTransform()->GetLocalPosition().x;
		});
		...
		if (is_last)
		{
			...
			vector<JGUIDockResizeLine*> r_b_line_array;
			r_b_line_array.assign(
				owner->m_ConnectedResizeLines[DD_Bottom].begin(),
				owner->m_ConnectedResizeLines[DD_Bottom].end());
			sort(r_b_line_array.begin(), r_b_line_array.end(), [&](JGUIDockResizeLine* l1, JGUIDockResizeLine* l2) -> bool
			{
				return l1->GetTransform()->GetLocalPosition().x < l2->GetTransform()->GetLocalPosition().x;
			});
	...
	if (type == 1)
	{
		...
		float pos_ratio = line_world_pos.x / prev_rect.width();
		ENGINE_LOG_INFO("ratio : {0}", pos_ratio);
		prevLeftRect = prev_rect;
		prevLeftRect.left = left_area.left;
		prevLeftRect.right = left_area.right;
		prevRightRect = prev_rect;
		prevRightRect.left = right_area.left;
		prevRightRect.right = right_area.right;
...
;		break;
	...
	if (containing_dock)
	{
		if (containing_dock->win && containing_dock->win->GetParent() != nullptr)
		{
...
void JGUIDock::CreateResizeLineByDirection(DockDirection dir, DockInfo* l_t_info, DockInfo* r_b_info)
{
	...
	JGUIDockResizeLine* resize_line = nullptr;

	resize_line = CreateJGUIComponent<JGUIDockResizeLine>("DockResizeLine");
...
void JGUIDock::ArrangeDockInfo(DockInfo* remove_info)
{
	...
	{
		JGUIDockResizeLine* line = remove_info->resizeLines[expected_remove_line_dir];
		if (line)
		{
	...
	if (expected_remove_line_dir != DD_Count)
	{
		JGUIDockResizeLine* delete_line = remove_info->resizeLines[expected_remove_line_dir];
		if (delete_line)
		{
			JGUIDockResizeLine* line = delete_line;
			switch (expected_remove_line_dir)
			{
			case DD_Left:
		...
		{
			if (line_pair.second == nullptr) continue;
			JGUIDockResizeLine* line = line_pair.second;
		...
		{
			delete_line->Reset();
			DestroyJGUIComponent(delete_line);
...
void JGUIDock::JGUIDockResizeLine::Tick(const JGUITickEvent& e)
{
	if (!m_IsResizeing && !GetCollider()->CheckInPoint(GetOwnerWindow()->GetMousePos()))
	{
		m_Line->SetColor(JColor::Black());
	}
...
void JGUIDock::JGUIDockResizeLine::AddBindDock(DockDirection dir, DockInfo* dock_info)
{
...
void JGUIDock::JGUIDockResizeLine::SizeUpdateByBindedWindow()
{
	auto dock_pos = GetParent()->GetTransform()->GetPosition();
	dock_pos.x = (float)(int)dock_pos.x;
	dock_pos.y = (float)(int)dock_pos.y;
...
JGUIRect JGUIDock::JGUIDockResizeLine::TopArea()
{
	JGUIRect win_rect;
	win_rect.top = 99999999;
	win_rect.bottom = -1;
...
JGUIRect JGUIDock::JGUIDockResizeLine::BottomArea()
{
	JGUIRect win_rect;
	win_rect.top = 99999999;
	win_rect.bottom = -1;
...
JGUIRect JGUIDock::JGUIDockResizeLine::RightArea()
{
	JGUIRect win_rect;
	win_rect.top = 99999999;
	win_rect.bottom = -1;
...
JGUIRect JGUIDock::JGUIDockResizeLine::LeftArea()
{
	JGUIRect win_rect;
	win_rect.top = 99999999;
	win_rect.bottom = -1;
