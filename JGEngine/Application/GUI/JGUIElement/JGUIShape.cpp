...
{
	JGUIElement::Awake();
	m_Renderer = CreateJGUIComponent<JGUIRenderer>();

	//m_Renderer->SetDrawingWindow(GetOwnerWindow());
	if (m_Renderer->GetRI())
	{
		m_Renderer->GetRI()->SetPriority(GetRISortingOrder());
	}


	InitRenderItem(m_Renderer->GetRI());
}
...
void JGUIShape::Tick(const JGUITickEvent& e)
{
	...
	//}
	//else
	//	m_Renderer->SetLayer(m_OwnerCanvas->GetLayer());

	//m_Renderer->SetRIPriority(GetRISortingOrder());
}

void JGUIShape::SetActive(bool is_active, bool is_hierarchy)
{
	JGUIElement::SetActive(is_active, is_hierarchy);
	m_Renderer->SetActive(is_active);
}
void JGUIShape::SetParent(JGUIElement* parent)
{
	JGUIElement::SetParent(parent);
	FindCanvas();
}
...
void JGUIShape::SetClipFlag(EJGUI_Clip_Flags flags)
{
	...
	if (m_Renderer && m_Renderer->GetRI() && m_Renderer->GetRI()->GetMaterial())
	{
		m_Renderer->GetRI()->GetMaterial()->SetValueAsInt("Clip", flag);
	}
	m_ClipFlag = flags;
}
...
void JGUIShape::CreateRI(JGUIWindow* drawing_win)
{
	...
	//m_Renderer->SetDrawingWindow(drawing_win);
	InitRenderItem(m_Renderer->GetRI());
	SetClipFlag(m_ClipFlag);
}

void JGUIShape::FindCanvas()
{
	m_OwnerCanvas = FindComponent<JGUICanvas>();
...
{
	m_Renderer = CreateJGUIComponent<JGUIWindowRenderer>();
}

void JGUIWindowTexture::Bind(const std::string& moduleKey)
{
	m_ModuleKey = moduleKey;
	m_Renderer->SetImage(m_ModuleKey);
}
void JGUIWindowTexture::UnBind()
{
	//m_Renderer->SetDrawingWindow(nullptr);
}



void JGUIImage::Awake()
{
	m_Renderer = CreateJGUIComponent<JGUIImageRenderer>();
}

void JGUIImage::SetImage(const std::string& path)
{
	m_Renderer->SetImage(path);
}
...
const JColor& JGUIImage::GetColor() const
{
	return m_Renderer->GetColor();
}
...
{
	m_Color = color;
	if (GetRenderItem() && GetRenderItem()->GetRI()) GetRenderItem()->GetRI()->GetMaterial()->SetValueAsFloat4("Color", m_Color);
...
{
	if (m_Thick != thick && GetRenderItem() && GetRenderItem()->GetRI())
	{
		auto size = GetTransform()->GetSize();
...
void JGUITextMesh::UpdateTextFlags()
{
	if (m_TextFlags & JGUI_Text_Flag_Border)
	{
		if (m_Border == nullptr)
		{
			m_Border = CreateJGUIElement<JGUIEmptyRectangle>("EmptyRectangle");
		}
