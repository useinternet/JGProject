...
void JGUIText::ProcessByTextFlag()
{
	if (m_TextFlags & JGUI_Text_Flag_Border)
	{
		if (m_Border == nullptr)
		{
			m_Border = CreateJGUIElement<JGUIEmptyRectangle>("TextBorder");
	...
	{
		if(m_Border) 	m_Border->SetActive(false);
...
void JGUIText::Reset()
{
	...
	for (auto& mesh : m_TextMeshs)
	{
		DestroyJGUIElement(mesh.second);
	}
	m_TextMeshs.clear();
}
...
JGUITextMesh* JGUIText::CreateTextMesh(int line)
{
	...
	auto size = GetTransform()->GetSize();
	auto mesh = CreateJGUIElement<JGUITextMesh>("TextMesh_" + to_string(m_Line));
