...
{
	JGUIElement::Awake();
	m_LineCanvas = CreateJGUIElement<JGUIElement>("ResizeLineCanvas");
	m_LineCanvas->CreateJGUIComponent<JGUICanvas>();




	m_DockTarget = CreateJGUIElement<JGUIElement>("DockTarget");
	m_DockTarget->CreateJGUIComponent<JGUICanvas>();
	m_DockTarget->GetTransform()->SetPivot(0.5f, 0.5f);
	for (int i = 0; i < DD_Count; ++i)
	{
		m_Docks[i] = CreateJGUIElement<JGUIImage>("Dock_" + to_string(i));
		m_Docks[i]->SetColor(JColor(1, 1, 1, 0.5F));
		m_Docks[i]->RegisterCollider(JGUI_Collider_Box);
...
void JGUIDock::SetActive(bool active, bool is_hierarchy)
{
	JGUIElement::SetActive(active, is_hierarchy);
...
;		break;
	...
	case DD_Center:
		...
		{
			// testcode
			//((JGUIForm*)(dragging_win))->GetTitleBar()->UnConnect();
			//((JGUIForm*)(containing_dock->win))->GetTitleBar()->BindWindow(dragging_win);
			// Tab 기능 구현 후 활성화
			// 비활성화
			return;
		}
...
void JGUIDock::CreateResizeLineByDirection(DockDirection dir, DockInfo* l_t_info, DockInfo* r_b_info)
{
	...
	resize_line = m_LineCanvas->CreateJGUIElement<JGUIDockResizeLine>("DockResizeLine");
...
void JGUIDock::ArrangeDockInfo(DockInfo* remove_info)
{
	...
	if (expected_remove_line_dir != DD_Count)
	{
		...
		{
			delete_line->Reset();
			DestroyJGUIElement(delete_line);
...
void JGUIDock::JGUIDockResizeLine::Awake()
{
	...
	m_Line = CreateJGUIElement<JGUIRectangle>("DockResizeLine");
