...
// DirectX data
static RE::RenderDevice* g_pRenderDevice = nullptr;
...
static std::string g_FontTextureName = "ImGui_DefaultFontTexture";
static D3D12_SHADER_RESOURCE_VIEW_DESC g_fontSrvDesc;
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    int width, height;
	io.Fonts->AddFontFromFileTTF("../../Contents/Engine/Fonts/DefaultFont.ttf", 15.0f, NULL, io.Fonts->GetGlyphRangesKorean());
    ...
    // Upload texture to graphics system
    {
		...
		g_fontSrvDesc = srvDesc;
		if (g_pFontTextureResource != NULL)
		{
			g_pRenderDevice->GUIUnAllocate(g_FontTextureName, &g_fontSrvDesc);
			g_pFontTextureResource->Release();
		}
        g_pFontTextureResource = pTexture;
		RE::Texture re_resource(g_FontTextureName);
		re_resource.SetD3DResource(g_pFontTextureResource, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
		g_pRenderDevice->RegisterGUITexture(re_resource);
		g_pRenderDevice->GUIAllocate(g_FontTextureName, &g_fontSrvDesc);
    }

    // Store our identifier
	auto font_gpu_address = g_pRenderDevice->GetGUIAllocation(g_FontTextureName, &g_fontSrvDesc);
	static_assert(sizeof(ImTextureID) >= sizeof(font_gpu_address), "Can't pack descriptor handle into TexID, 32-bit not supported yet.");
	io.Fonts->TexID = (ImTextureID)font_gpu_address;
}
...
bool ImGui_ImplDX12_Init(RE::RenderDevice* device)
{
	...
	auto device_desc = device->GetDesc();
	g_pRenderDevice = device;
	g_pd3dDevice = device->GetD3DDevice();
	g_RTVFormat = device_desc.format;
	g_pFrameResources = new FrameResources[device_desc.bufferCount];
	g_numFramesInFlight = device_desc.bufferCount;
	g_frameIndex = UINT_MAX;

    // Create buffers with a default size (they will later be grown as needed)
    for (uint32_t i = 0; i < device_desc.bufferCount; i++)
    {
        FrameResources* fr = &g_pFrameResources[i];
        fr->IndexBuffer = NULL;
...
void ImGui_ImplDX12_NewFrame()
{
	...
	auto& io = ImGui::GetIO();
	io.Fonts->TexID = (ImTextureID)g_pRenderDevice->GetGUIAllocation(g_FontTextureName, &g_fontSrvDesc);
}
