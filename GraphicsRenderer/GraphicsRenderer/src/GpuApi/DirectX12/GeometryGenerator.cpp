...
namespace GR
{
	namespace Dx12
	{
		GeometryGenerator::MeshData GeometryGenerator::CreateBox(float width, float height, float depth, UINT32 numSubdivisions)
		{
			...
			// BBox
			BoundingBox::CreateFromPoints(meshData.BBox, XMLoadFloat3(&float3(-w2, -h2, -d2)), XMLoadFloat3(&float3(w2, h2, d2)));
		...
		GeometryGenerator::MeshData GeometryGenerator::CreateSphere(float radius, UINT32 sliceCount, UINT32 stackCount)
		{
			...
			for (UINT32 i = 1; i <= stackCount - 1; ++i)
			{
				...
				for (UINT32 j = 0; j <= sliceCount; ++j)
				{
					...
					sim_vector T = XMLoadFloat3(&v.tangent);
			...
			BoundingSphere BSphere(float3(0, 0, 0), radius);
			BoundingBox::CreateFromSphere(meshData.BBox, BSphere);
		...
		GeometryGenerator::MeshData GeometryGenerator::CreateGrid(float width, float depth, UINT32 m, UINT32 n)
		{
			...
			for (UINT32 i = 0; i < m; ++i)
			{
				...
				for (UINT32 j = 0; j < n; ++j)
				{
					...
					meshData.Vertices[i*n + j].tangent = float3(1.0f, 0.0f, 0.0f);

					// Stretch texture over grid.
					meshData.Vertices[i*n + j].tex = float2(j*du, i*dv);
				}
			}
			...
			// BBox
			BoundingBox::CreateFromPoints(meshData.BBox, XMLoadFloat3(&float3(-halfWidth, 0.0f, -halfDepth)), XMLoadFloat3(&float3(halfWidth, 0.0f, halfDepth)));
