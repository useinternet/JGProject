...
float4 PS(VertexOut pin) : SV_Target
{
	...
	float2 TexC0 = texC + float2(0.01f, 0.04f) * gTotalTime;
	float4 normalMap0 = gTexture[matData.TextureIndex[1]].Sample(gsamAnisotropicWrap, TexC0);
	float3 bumpNormalW0 = NormalSampleToWorldSpace(normalMap0.rgb, normalW, pin.TangentW);

	float2 TexC1 = texC + float2(0.003f, 0.001f) * gTotalTime;
	float4 normalMap1 = gTexture[matData.TextureIndex[2]].Sample(gsamAnisotropicWrap, TexC1);
	float3 bumpNormalW1 = NormalSampleToWorldSpace(normalMap1.rgb, normalW, pin.TangentW);

	normalW = normalize( (bumpNormalW0 + bumpNormalW1));
	// SSAO 계산
