...
float4 main(PixelInputType input) : SV_TARGET
{
	...
	float4 Normal_SpecPow = T_Normal_SpecPow(ClampSampler, input.tex);


	// °ª ÃßÃâ
	float3 Albedo        = T_Albedo(ClampSampler, input.tex).xyz;
	float3 SpecularColor = T_Specular(ClampSampler, input.tex).xyz;
