...
{
	return static_cast<float>(mSetting.Width) / mSetting.Height;
}
...
void DxCore::Set4xMsaaState(bool value)
{
	if (m4xMsaaState != value)
	{
		...
		CreateSwapChain();
		OnResize(mSetting.Width, mSetting.Height);
	}
}
...
void DxCore::OnResize(const int width, const int height)
{
	assert(mDevice);
	assert(mSwapChain);
	...
	ThrowIfFailed(mSwapChain->ResizeBuffers(
		...
		mSetting.Width, mSetting.Height,
		mBackBufferFormat,
	...
	depthStencilDesc.Width = mSetting.Width;
	depthStencilDesc.Height = mSetting.Height;
	...
	mScreenViewport.Width = static_cast<float>(mSetting.Width);
	mScreenViewport.Height = static_cast<float>(mSetting.Height);
	mScreenViewport.MinDepth = 0.0f;
	mScreenViewport.MaxDepth = 1.0f;

	mScissorRect = { 0, 0, mSetting.Width, mSetting.Height };

	XMMATRIX P = XMMatrixPerspectiveFovLH(0.25f * MathHelper::Pi, AspectRatio(), mSetting.NearZ, mSetting.FarZ);
...
ID3D12Resource* DxCore::DepthStencilBuffer() const
{
...
void DxCore::CreateSwapChain()
{
	...
	sd.BufferDesc.Width = mSetting.Width;
	sd.BufferDesc.Height = mSetting.Height;
